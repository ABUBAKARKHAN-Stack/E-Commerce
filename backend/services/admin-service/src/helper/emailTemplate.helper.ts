    ...
    <html>
      ...
      <body>
        <div class="container">
          ...
          <a href="http://localhost:${purpose.toLowerCase().includes("email") ? "3005" : "5173"}/admin${purpose.toLowerCase().includes("email") ? `/verify/${email}/${token}` : `/reset-password?email=${email}&token=${token}`}" target="_blank" class="verify-btn">
