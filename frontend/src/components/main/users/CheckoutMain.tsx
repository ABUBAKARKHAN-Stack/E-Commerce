...
const CheckoutMain = () => {
  ...
  const [activeTab, setActiveTab] =
    useState<CheckoutTabsType>("shipping-address");
  ...
  return (
    <main className="relative h-full w-full min-w-screen overflow-x-hidden border-b-2 bg-gradient-to-b from-[#F3F4F6] via-[#E5E7EB] to-[#F3F4F6] py-10 backdrop-blur-xl dark:bg-gradient-to-b dark:from-[#1B1B1F] dark:via-[#27272A] dark:to-[#1B1B1F]">
      <Layout>
        ...
        />
        <section className="shipping-method-section checkout-summary-section payment-section shipping-address-section mt-10 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4 md:gap-0">
            {[
              ...
              <div
                key={step}
                className="flex min-w-[80px] flex-1 flex-col items-center gap-y-2"
              >
                <div
                  className={`flex size-12 items-center justify-center rounded-full transition-colors duration-100 ease-linear ${activeTab === value ? "bg-cyan-500 text-cyan-100 dark:bg-orange-500 dark:text-orange-200" : "bg-cyan-100 text-cyan-500 dark:bg-orange-200 dark:text-orange-500"} font-semibold`}
                >
                  {step}
                </div>
          ...
          <div className="flex items-center justify-between">
            {["shipping-method", "payment", "checkout-summary"].includes(
              activeTab,
            ) && <Button onClick={handlePreviousTab}>Previous Tab</Button>}
