...
const UserOrdersMain = () => {
  ...
  const { getAllOrders, ordersData, setOrdersData, ordersCount } =
    useOrderContext();
  ...
  useEffect(() => {
    ...
    const parsedOrderFilterSortParams = orderFilterSortParams
      ? JSON.parse(orderFilterSortParams)
      : {};
  ...
  }, [status, orderId, sortBy, page, limit, isInitialized]);
          ...
          <div className="space-y-10 px-4">
            <BlurFade delay={1} duration={0.5} blur="50px" direction="down">
              <DashboardMainHeader
                ...
                subIcon={
                  <Package className="size-5 text-cyan-100 dark:text-orange-100" />
                }
                subText="Track, manage, and review your recent purchases here."
                animateClassName="orders-header"
              />
            </BlurFade>
            <BlurFade delay={1.5} className="relative z-10" direction="down">
              <OrderFilterPanel
            ...
            >
              <OrderCard totalOrders={ordersData} filterError={filterError} />
            </BlurFade>
