...
import ViewProductSkeleton from "../../Skeleton&Loaders/skeleton/ViewProductSkeleton";
import ViewProductErrorState from "./errorStates/ViewProductErrorState";
...
const ViewProduct: FC<Props> = ({ isOpen, onOpenChange, productId }) => {
  ...
  useEffect(() => {
    if (product) {
      setCurrentImageSrc(product?.thumbnails[0]);
    }
  }, [product]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="h-[95%] max-h-[550px] w-full overflow-auto sm:!max-w-[610px] md:!max-w-2xl lg:!max-w-3xl">
        ...
        {/* Loading Skeleton */}
        {isLoading && <ViewProductSkeleton />}

        {/* Error State */}
        {isError && (
          <ViewProductErrorState
